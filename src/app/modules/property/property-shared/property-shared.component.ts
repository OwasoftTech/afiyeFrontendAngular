...
export class PropertySharedComponent implements OnInit {
  ...
  getData() {
    console.log(
      "PropertySharedComponent -> getData -> this.requestService.getLoggedInUser()",
      this.requestService.getLoggedInUser()
    );
    let params = {
      pagination: 1,
      page: this.activePage,
      per_page: this.limit,
      user_id: this.requestService.getLoggedInUser()
        ? this.requestService.getLoggedInUser().id
        : null,
    };
    if (
      this.requestService.getLoggedInUser() &&
      !this.route.snapshot.data["isFeatured"]
    ) {
      params["user_id_by"] = 1;
    }
    if (this.route.snapshot.data["isFavorite"]) {
    ...
    params["user_id"] = this.requestService.getLoggedInUser()
      ? this.requestService.getLoggedInUser().id
      : null;
    this.requestService
      .sendRequest(this.url, "GET", removeEmptyKeysFromObject(params))
      .subscribe(
        (res) => {
          if (res.status) {
            ...
            console.log(
              "PropertySharedComponent -> getData -> this.total",
              this.total
            );
            setTimeout(() => {
              oneGridOwlCarousel();
  ...
  markFavorite(property) {
    ...
    this.requestService
      .sendRequest(PropertyUrls.PROPERTY_FAVORITE_POST, "POST", params)
      .subscribe(
        (res) => {
  ...
  markUnFavorite(property) {
    ...
    let params = {
      user_id: this.requestService.getLoggedInUser().id,
      property_id: this.route.snapshot.data["isFavorite"]
  ...
  preview(media) {
    ...
    return ft.length
      ? ft[0]?.base_path + "/" + ft[0]?.system_name
      : media[0]?.base_path + "/" + media[0]?.system_name;
  }
}
